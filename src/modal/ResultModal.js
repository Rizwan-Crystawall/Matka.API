...
const insertOrUpdateResults = async (payload, userId) => {
...
const fetchBetsWithDigitStats = async (digit, mmid, isClosedType) => {
  const sql = `
    SELECT 
      b.id AS bet_id, 
      b.user_id, 
...
const updateWinnerWallet = async (userId, profitPlusStake, stake) => {
  await execute(
    `UPDATE wallet SET wallet_balance = wallet_balance + ?, exposure = exposure - ? WHERE user_id = ?`,
    [profitPlusStake, stake, userId]
  );
};

const updateLoserWallet = async (userId, stake) => {
...
const closeBets = async (mmid, isClosedType) => {
  await execute(
...
const getAllResults = async () => {
  const sql = `
 SELECT res.id, mkt.name as marketname,m.name as matchname, DATE_FORMAT(m.draw_date, '%d-%m-%Y') AS matchdate, CASE WHEN m.is_active = 1 THEN 'Active' ELSE 'Inactive' END AS status FROM results res, matches_type_mapping mtm, matches m,markets mkt WHERE mtm.id=res.match_map_id AND m.id=mtm.match_id AND m.market_id =mkt.id ORDER by res.id DESC;
  `;
